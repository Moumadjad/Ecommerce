...
- **Cart & checkout** — done. Cart is server-side (`Cart` model, one per user, `/api/cart`) for logged-in users, so it persists across devices/sessions; guests still get a localStorage cart, which merges into the server cart on login/register. Checkout derives its items strictly from the server cart (never from client-supplied input) and clears the cart after the order is created. Every order gets a sequential, human-readable `orderNumber` (e.g. `ORD-000042`), generated via an atomic `Counter` document so it's safe under concurrent checkouts.
